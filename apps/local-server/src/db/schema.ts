...
const CODING_AGENT_SETTINGS_SQL = `
  CREATE TABLE coding_agent_settings (
    ...
    provider TEXT NOT NULL DEFAULT 'fake' CHECK (provider IN ('fake', 'codex', 'claudecode', 'openai', 'gemini', 'deepseek', 'openrouter')),
...
function ensureProviderSettingsTable(db: GraphDatabase, tableName: string, createSql: string): void {
  ...
  if (!sql.includes("'codex'") || !sql.includes("'deepseek'") || !hasCodexSettingsColumns) {
