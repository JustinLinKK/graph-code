...
describe("SQLite graph repository", () => {
  ...
  it("saves settings with masked views and records agent status history", () => {
    ...
    it("reconciles orphaned running workflows, items, and agent runs on startup", () => {
      ...
      const runningItem = runningWorkflow.items.find((item) => item.status === "pending")!;
      repo.updateCodingWorkflowItem({ itemId: runningItem.id, status: "running" });
      ...
      expect(repo.markInterruptedWork()).toEqual({ workflows: 1, items: 1, agentRuns: 2 });
      expect(repo.getCodingWorkflow(runningWorkflow.id).status).toBe("failed");
      expect(repo.getCodingWorkflow(runningWorkflow.id).items.find((item) => item.id === runningItem.id)?.status).toBe("failed");
      expect(repo.getCodingWorkflow(previewWorkflow.id).status).toBe("preview");
      expect(repo.getAgentRun(runningRun.id)).toMatchObject({ status: "failed", error: "Interrupted by server restart." });
      expect(repo.getAgentRun(queuedRun.id)).toMatchObject({ status: "failed", error: "Interrupted by server restart." });
